...
public class Algorithm<N extends Number, T extends Gauss<N, T>> {
    ...
    public void calculate() throws NullPointerException, ArithmeticException{
        if (list == null){
            throw new NullPointerException("method_of_least_squares.LinearSystem<N, T> instance equal null");
        }
        if (!checkSystem(list)){
            throw new ArithmeticException("Incorrect system for method_of_least_squares.Gauss Method");
        }
